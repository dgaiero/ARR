...
\pagestyle{fancy}
\fancyhf{}
\lhead{Apartment Rules and Regulations}
\rhead{\thepage}
...
\begin{enumerate}
	...
	\item Footwear can either be placed in the designated footwear storage bin or in one’s University appointed bedroom.
	...
	\item Footwear can be removed in one's university appointed bedroom unless the under of the footwear is wet, then it shall be removed at the entrance and may be placed in one's university appointed bedroom.
...
This section covers the protocol with respect to cleaning and maintain the apartment kitchen. In essence, there is no reason a dish should be left in the sink for longer than twenty-four hours. 
\begin{enumerate}
	...
	\textbf{\item Any person that leaves an item in the sink for longer than twenty-four hours shall put \$3 in the rainy-day fund. (Amount subject to change)}
...
With this in mind, it seems that the only appropriate form of punishment (at least for the lesser violations) is some sort of baking. Any violator must bake some sort of delicious treat.\smiley
\\\\
Some violations require the offender to deposit money into the rainy day fund (see \S \ref{Rainy Day Fund}). The amount that must be deposited per violation is stated in each section. Follow the guidelines per section for more information.
\\\\
